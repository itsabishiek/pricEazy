...
export async function getProductForBannerInternal({
  id,
  countryCode,
  url,
}: {
  ...
  const data = await db.query.ProductTable.findFirst({
    where: (products, { and, eq }) =>
      and(eq(products.id, id), eq(products.url, removeTrailingSlash(url))),
    columns: {
      id: true,
      clerkUserId: true,
    },
          ...
          countryGroup: {
            ...
            with: {
              countries: {
                ...
                limit: 1,
                where: (countries, { eq }) => eq(countries.code, countryCode),
              },
            },
          },
        },
