...
export const ProductCustomizationTable = pgTable("product_customizations", {
  ...
  locationMessage: text("location_message")
    .notNull()
    .default(
      "Hey there! It looks like you're from <b>{country}</b>. Enjoy pricing tailored to your location with Purchasing Power Parity. Use code <b>{coupon}</b> for an exclusive <b>{discount}%</b> off!"
    ),
  backgroundColor: text("background_color").notNull().default("#e1b505"),
  textColor: text("text_color").notNull().default("#ffffff"),
  fontSize: text("font_size").notNull().default("1rem"),
  borderRadius: text("border_radius").notNull().default("0px"),
